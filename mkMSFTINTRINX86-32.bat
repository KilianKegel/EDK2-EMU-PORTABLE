...
call %DDKROOTDIR%LaunchBuildEnv.cmd x86

popd

rem
rem automatically create MSCFINTRINx86-32.lib 
rem 

rem cleanup TEMP directory
set DELETE=lldiv.obj lldvrm.obj llmul.obj llrem.obj llshl.obj llshr.obj ulldiv.obj ulldvrm.obj ullrem.obj ullshr.obj _ftol2.obj _ftol3_.obj memcpy.obj memcmp.obj __isa_available.obj
for %%d in (%DELETE%) do (
    if exist %TEMP%\%%d (
        attrib -r %TEMP%\%%d
        del %TEMP%\%%d
...
rem
rem isolate INT64 intrinsics, need leading "\" to differtiate ll*.obj from ull*.obj
rem extract *.obj to %TEMP%
rem 
set OBJS=lldiv.obj lldvrm.obj llmul.obj llrem.obj llshl.obj llshr.obj ulldiv.obj ulldvrm.obj ullrem.obj ullshr.obj _ftol2_.obj _ftol3_.obj

for  %%o in (%OBJS%) do (

    for /F %%f in ('lib /nologo /list %LIBCMT_LIB% ^| find "\%%o"') do (
        lib /nologo /extract:%%f %LIBCMT_LIB% /out:%TEMP%\%%o
    )
)
...
echo unsigned int _fltused = 9875; > %temp%\_fltused.c
...
echo lib /nologo %OBJS% /out:%~dp0\EDK2\CONF\MSFTINTRINX86-32.lib
lib /nologo %OBJS% /out:%TEMP%\MSFTINTRINX86-32.lib

move %TEMP%\MSFTINTRINX86-32.lib %~dp0\EDK2\CONF\MSFTINTRINX86-32.lib

popd

echo lib /nologo /list %~dp0\EDK2\CONF\MSFTINTRINX86-32.lib
lib /nologo /list %~dp0\EDK2\CONF\MSFTINTRINX86-32.lib
